...
# as grammes
poids_nouveau_ne_min = configs["poids_nouveau_ne_min"]
poids_nouveau_ne_max = configs["poids_nouveau_ne_max"]
...
# as weeks
age_gestationnel_min = configs["age_gestationnel_min"]
age_gestationnel_max = configs["age_gestationnel_max"]
...
def generate_yearly_fake_data(year, num_records):
    ...
    min_age = configs["min_age"] # min age of patient
    max_age = configs["max_age"] # max age of patient
    ...
    value_ranges = {
        ...
        "Date_naissance": {"min_age": min_age, "max_age": max_age},
    ...
    for column_name, length, data_type, fill_type, liste in columns_info:
        if column_name == "Date_entree_unite_medicale":
            ...
            data[column_name] = [
                format_data(fake.date_of_birth(minimum_age=min_age, maximum_age=max_age).strftime("%d%m%Y"), length, fill_type)
                for _ in range(num_records)
            ]
            # data[column_name] = [
